...
const Cryp = () => {
    ...
    const [sha, setSha] = useState(''); // 단방향 sha256해시
    ...
    const decryptFn = () => {
        try {
            ...
            alert("복호화에 실패했습니다. 암호키를 확인하세요")
        } finally {
            alert("암호키 관리에 주의해주세요")
        }
    }
    ...
    return (
        <>
            ...
            <div>{(!!encrypt) && `암호화 후 : ${encrypt} (글자수 : ${encrypt.length})`}</div><br />
            <hr />
            <div>복호화 암호키
            ...
            <div>{`복호화 된 데이터 ID : ${decrypt.id} / pw : ${decrypt.pw}`}</div>
            <hr />
            <button onClick={sha256Fn} disabled={!(inid && inpw && skey)}>단방향 암호화 하기</button>
            <h3>{`SHA256 암호화 : ${sha} (길이 ${sha.length})`}</h3>
        </>
    )
}
